...
def to_device(data, device):
    if len(data) == 13:
        ...
        dberts = torch.from_numpy(dberts).to(device)
...
def synth_one_sample(targets, predictions, vocoder, model_config,
    ...
    if preprocess_config["preprocessing"]["pitch"][
            "feature"] == "phoneme_level":
        pitch = targets[9][0, :src_len].detach().cpu().numpy()
        pitch = expand(pitch, duration)
    else:
        pitch = targets[9][0, :mel_len].detach().cpu().numpy()
    if preprocess_config["preprocessing"]["energy"][
            "feature"] == "phoneme_level":
...
def synth_samples(targets, predictions, vocoder, model_config,
    ...
    lengths = predictions[9] * preprocess_config["preprocessing"]["stft"][
        "hop_length"]
    wav_predictions = vocoder_infer(mel_predictions,
                                    vocoder,
                                    model_config,
                                    preprocess_config,
                                    lengths=lengths)
